...
static bool SmokeSceneBuffersChanged(const RtSmokeSceneBufferHandles& oldBuffers, const RtSmokeSceneBufferHandles& newBuffers)
{
    return
        ...
        oldBuffers.skinnedHitRouteRecordBuffer != newBuffers.skinnedHitRouteRecordBuffer ||
        oldBuffers.skinnedHitRouteTriangleBuffer != newBuffers.skinnedHitRouteTriangleBuffer ||
...
RtSmokeBindingBuildResult CreateSmokeBindingResources(const RtSmokeBindingBuildDesc& desc, RtSmokeMaterialTableBuild& materialTable)
{
    ...
    if (!desc.device || !desc.bindingLayout || !desc.tlas || !desc.outputTexture || !desc.accumulationTexture || !desc.restirPTReflectionTexture || !desc.rrInputColorTexture || !desc.motionVectorTexture || !desc.rrMotionVectorTexture || !desc.motionVectorMaskTexture || !desc.rrGuideAlbedoTexture || !desc.rrGuideSpecularAlbedoTexture || !desc.rrGuideNormalRoughnessTexture || !desc.rrGuideDepthTexture || !desc.rrGuideHitDistanceTexture || !desc.rrGuideResetMaskTexture || !desc.rrGuidePositionTexture || !desc.fallbackTexture || !desc.skyEnvironmentCube || !desc.constantsBuffer || !desc.boundsOverlayLineBuffer || !desc.unifiedPtPrimaryReceiverBuffer || !desc.sampler || !desc.buffers.IsValid() || !desc.primarySurfaceHistoryBuffers.IsValidFor(desc.primarySurfaceHistoryBuffers.width, desc.primarySurfaceHistoryBuffers.height))
    {
...
RtSmokeSceneResourceCommitDesc CreateSmokeSceneResourceCommitDesc(const RtSmokeSceneResourceCommitBuildDesc& desc)
{
    ...
    commitDesc.unifiedPtEmissiveLookupCount = desc.unifiedPtEmissiveLookupCount;
    return commitDesc;
}
    ...
    nvrhi::ShaderLibraryHandle skinnedHitShaderLibrary = nullptr)
{
    ...
    nvrhi::ShaderHandle skinnedClosestHit = closestHit;
    nvrhi::ShaderHandle skinnedAnyHit = anyHit;
...
bool PathTracePrimaryPass::HasRetainableRayTracingSmokeScenePackage() const
{
    ...
    buffers.skinnedHitRouteRecordBuffer = m_smokeSkinnedHitRouteRecordBuffer;
    buffers.skinnedHitRouteTriangleBuffer = m_smokeSkinnedHitRouteTriangleBuffer;
...
RtRetiredSmokeScenePackage PathTracePrimaryPass::CaptureRetiredRayTracingSmokeScenePackage() const
{
    ...
    package.buffers.skinnedHitRouteRecordBuffer = m_smokeSkinnedHitRouteRecordBuffer;
    package.buffers.skinnedHitRouteTriangleBuffer = m_smokeSkinnedHitRouteTriangleBuffer;
...
void PathTracePrimaryPass::ResetRayTracingSmokeSceneResources()
{
    ...
    m_smokeSkinnedHistoryOwnerGateLast = -1;
    ...
    m_smokeSkinnedHitRouteRecordBuffer = nullptr;
    m_smokeSkinnedHitRouteTriangleBuffer = nullptr;
    ...
    m_smokeUnifiedPtEmissiveLookupCount = 0;
...
void PathTracePrimaryPass::CommitRayTracingSmokeSceneResources(const RtSmokeSceneResourceCommitDesc& desc)
{
    ...
    m_smokeSkinnedHitRouteRecordBuffer = desc.buffers.skinnedHitRouteRecordBuffer;
    m_smokeSkinnedHitRouteTriangleBuffer = desc.buffers.skinnedHitRouteTriangleBuffer;
    ...
    m_smokeUnifiedPtEmissiveLookupCount = desc.unifiedPtEmissiveLookupCount;
